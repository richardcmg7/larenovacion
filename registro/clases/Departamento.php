...
class Departamento
{
	...
	function guardar($obj)
	{
		...
		$consulta = "SELECT count(*) as cantidad FROM departamentos WHERE eliminado=1 AND nombre='" . $obj->nombre . "' ";
		$cnx->consultar($consulta);
		$row = mysqli_fetch_array($cnx->resultado);
		$existe = $row['cantidad'];

		if ($existe == 0) {
			$sql = "insert into departamentos (nombre,estado,eliminado,pais_id) values ('" . $obj->nombre . "','a',1,1)";
	...
	function consultar($obj)
	{
		...
		FROM departamentos WHERE eliminado=1 ORDER BY nombre ASC";
	...
	function editar($obj)
	{
		...
		$consulta = "SELECT count(*) as cantidad FROM departamentos WHERE id!=" . $obj->id . " AND eliminado=1 AND nombre='" . $obj->nombre . "' ";
		$cnx->consultar($consulta);
		$row = mysqli_fetch_array($cnx->resultado);
		$existe = $row['cantidad'];

		if ($existe == 0) {
			$sql = "UPDATE departamentos SET nombre='" . $obj->nombre . "',estado='" . $obj->estado . "' WHERE id=" . $obj->id . " ";
			if (!$cnx->Sentencia($sql)) {
				$num_error = $num_error + 1;
	...
	function eliminar($obj)
	{
		...
		$sql = "UPDATE departamentos SET eliminado=0 WHERE id=" . $obj->id . " ";
		if (!$cnx->Sentencia($sql)) {
			$num_error = $num_error + 1;
