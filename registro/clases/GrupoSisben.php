...
class GrupoSisben{
	...
	function guardar($obj){
	    ...
	    $consulta="SELECT count(*) as cantidad FROM grupos_sisben WHERE eliminado=1 AND nombre='".$obj->nombre."' ";
		$cnx->Consultar($consulta);
		$row=mysqli_fetch_array($cnx->resultado);
		$existe=$row['cantidad'];

		if($existe == 0){
			$sql="insert into grupos_sisben (nombre,estado,eliminado) values ('".$obj->nombre."','a',1)";
	...
	function consultar($obj) {
		...
		FROM grupos_sisben WHERE eliminado=1 ORDER BY nombre ASC";
	...
	function editar($obj){
	    ...
	    $consulta="SELECT count(*) as cantidad FROM grupos_sisben WHERE id!=".$obj->id." AND eliminado=1 AND nombre='".$obj->nombre."' ";
		$cnx->Consultar($consulta);
		$row=mysqli_fetch_array($cnx->resultado);
		$existe=$row['cantidad'];

		if($existe == 0){
		    $sql="UPDATE grupos_sisben SET nombre='".$obj->nombre."',estado='".$obj->estado."' WHERE id=".$obj->id." ";
			if (!$cnx->Sentencia($sql)){
				$num_error=$num_error+1;
	...
	function eliminar($obj){
	    ...
	    $sql="UPDATE grupos_sisben SET eliminado=0 WHERE id=".$obj->id." ";
		if (!$cnx->Sentencia($sql)){
			$num_error=$num_error+1;
	...
	function consultarPublico($obj) {
		...
		$sql = "SELECT id,nombre FROM grupos_sisben WHERE eliminado=1 AND estado='a' ORDER BY nombre ASC";
